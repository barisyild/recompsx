...
**Sound** (on by default) is the audio output. Off, the page creates no `AudioContext` and
...
**Speed limit** (on by default) is the pacing. Off, `host.unpaced` is true and the loop in
...
**WebGL rendering** (on by default where WebGL2 exists) hands the PlayStation's primitives to
...
primitive draws, "check" skips what is marked. Every vertex carries its primitive's texture
page, depth, CLUT, window and blend mode, and the three adding blend equations share one GL
blend state (source factor ONE, destination factor the fragment's alpha, colour pre-scaled), so
texture and palette switches end no batch and a vblank of gameplay is one to three draw calls
rather than about two hundred; a mode-2 (subtracting) primitive is a batch of its own, drawn in
two passes when textured. The framebuffer texture's alpha stays 1 and the blit writes 1: the
canvas is unpremultiplied, and any lower alpha would darken the pixel on the page. Headless Node runs have no host and never take
