#!/usr/bin/env bash
# Compare scalar-only and region emission on a synthetic multi-block MIPS loop.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
source scripts/env.sh
OUT=out/_regions_bench
mkdir -p "$OUT"
...
root = Path('out/_regions_bench')
results = {'host': platform.platform(), 'iterations_per_run': 25000000}
oracle = None
for target, command in [('js', ['node', str(root / 'bench.js')]),
                        ('cpp', [str(root / 'build/recompsx')])]:
    timings = {mode: [] for mode in ('scalar', 'regions')}
    output = None
    for sample in range(6):
        order = ('scalar', 'regions') if sample % 2 == 0 else ('regions', 'scalar')
        for mode in order:
    ...
    results[target] = {'seconds': medians, 'samples': timings,
                       'speedup': medians['scalar'] / medians['regions'],
                       'output': output.strip()}
